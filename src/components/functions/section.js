...
export default class TopNav extends React.Component {
  render() {
    ...
    const keys = Object.keys(jsonData.diseases);
    let immediate = [];
    let medium = [];
    let low = [];
    ...
    const list1 = immediate.map(disease => (
      <Card
        key={disease.replace(/\s/g, '')}
        text={disease}
        urgency="3 hours"
    ...
    const list2 = medium.map(disease => (
      <Card
        key={disease.replace(/\s/g, '')}
        text={disease}
        urgency="24 hours"
